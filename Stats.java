...
public class Stats
{
  ...
  HP formula = level+10 + ( (basestat+IV)x2 + [sqrt(EV)/4]) x level ) /100 )
  ...
  */
  ...
  {
    Random r = new Random();
    return r.nextInt(15 - 0 + 1);
  }
  public static int statCalculator(String stattype, Pokemon p)
  {
    int finalStat;
    Stats s = getPokemonStat(p.getName());
    switch(stattype)
    {
      case "attack":
         finalStat = (int)(5 + (( 2*(s.getAttack() + ivGenerator()) + ((Math.sqrt(p.getEV()))/4))*Pokemon.getLevel())/100);
         break;
      case "defense":
         finalStat = (int)(5 + (( 2*(s.getDefense() + ivGenerator()) + ((Math.sqrt(p.getEV()))/4))*Pokemon.getLevel())/100);
         break;
      case "speed":
         finalStat = (int)(5 + (( 2*(s.getSpeed() + ivGenerator()) + ((Math.sqrt(p.getEV()))/4))*Pokemon.getLevel())/100);
         break;
      case "special":
         finalStat = (int)(5 + (( 2*(s.getSpecial() + ivGenerator()) + ((Math.sqrt(p.getEV()))/4))*Pokemon.getLevel())/100);
         break;
      case "hp":
         finalStat = (int)(10+Pokemon.getLevel() + (( 2*(s.getHP() + ivGenerator()) + ((Math.sqrt(p.getEV()))/4))*Pokemon.getLevel())/100);
         break;
      default:
         finalStat = 0;
         break;
  ...
  public static Stats pokemonStats(Pokemon p)
  {
    return new Stats(statCalculator("hp",p),statCalculator("attack",p),statCalculator("defense",p),statCalculator("speed",p),statCalculator("special",p));
  }
  ...
  public static void main(String[] args)
  {
    ...
    System.out.println(bulbaS.getSpeed());
    System.out.println(bulbaS.getSpecial());
  }
}
